...
def load_character_images(character_name: str) -> list[tuple[str, Image.Image]]:
    ...
    return [(str(p), Image.open(p)) for p in img_paths]
...
def generate_styled(
    ...
    char_images: list[tuple[str, Image.Image]] = []
    if characters:
    ...
    # If characters are included, add instruction to use them as visual reference
    if char_images:
    ...
    # Build contents: instruction first, then style references, then character references
    contents: list = [styled_prompt] + [img for _, img in ref_images] + [img for _, img in char_images]
...
def main():
    ...
    try:
        ...
        char_count = 0
        if args.character:
            for char_name in args.character:
                char_count += len(load_character_images(char_name))
        ...
        if args.character:
            print(f"Character references: {', '.join(args.character)} ({char_count} images)")
        if text:
